...
const TenantErrorResponseSchema = registry.register('TenantErrorResponse', z.object({
    success: z.literal(false),
    message: z.string().openapi({ example: 'Tenant not found' }),
    errorCode: z.string().optional(),
    errors: z.array(z.any()).optional(),
}));
const TenantSchema = registry.register('TenantEntity', z.object({
...
const TenantWithRoleSchema = registry.register('TenantWithRole', TenantSchema.extend({
    role: z.enum(['owner', 'admin', 'member']).openapi({ example: 'owner' }),
}));
const TenantSuccessEnvelopeSchema = registry.register('TenantSuccessEnvelope', z.object({
...
registry.registerPath({
    ...
    description: 'Returns tenant detail with role resolved for current user membership when available.',
    tags: ['Tenant'],
    security: [{ bearerAuth: [] }, { cookieAuth: [] }],
    request: {
        params: z.object({
